...
Request for Quote handles trades with off-chain negotiations and on-chain settlement ([trades](https://about.airswap.io/technology/request-for-quote)). Market makers are called Makers who run servers to fulfil orders. Counterparties to makers are called Takers, who wish to trade tokens.

RFQ can be seen as a peer-to-peer system. The protocol’s smart contracts focus on on-chain settlement of trades via "atomic" swaps using smart contracts. Price discovery and negotiation are made off-chain via RPC ([remote procedure call](https://en.wikipedia.org/wiki/Remote_procedure_call)), which is scalable and resistant to AMM issues related to front running and miner extractable value.

This model exists because of Ethereum’s constraints and the issues around large orders on AMMs mentioned previously.

## Supplying Liquidity

Liquidity is provided by Makers who run [Servers](https://about.airswap.io/technology/makers). These servers are discoverable via the [Registry Protocol](https://about.airswap.io/technology/discovery) which allows for price discovery.
...
This is done via [atomic swaps](https://twitter.com/airswap/status/1380219827546112004) using a Swap contract. They are called atomic because they settle or they don’t.
